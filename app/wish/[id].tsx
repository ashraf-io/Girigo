...
export default function WishDetailScreen() {
  ...
  const timeLabel = !wish ? '' : (isExpired 
    ? 'EXPIRED' 
    : timeLeft.days > 0 
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await (await db).runAsync(
        'UPDATE wishes SET title = ?, description = ?, deadline = ? WHERE id = ? AND userId = ?',
        [editTitle.trim(), editDescription.trim(), editDeadline.toISOString(), wish.id, currentUserId]
      );
  ...
  const handleExtendDeadline = async (hours: number) => {
    ...
    try {
      const db = await import('../../src/db/database').then(m => m.getDatabase());
      await (await db).runAsync(
        'UPDATE wishes SET deadline = ? WHERE id = ? AND userId = ?',
        [newDeadline.toISOString(), wish.id, currentUserId]
      );
  ...
  const handleExtendDeadlineCustomDate = async (newDate: Date) => {
    ...
    try {
      ...
      await (await db).runAsync(
        'UPDATE wishes SET deadline = ? WHERE id = ? AND userId = ?',
        [newDate.toISOString(), wish.id, currentUserId]
      );
  ...
  const handleComplete = async () => {
    ...
    Alert.alert('Mark Complete', 'Are you sure you want to mark this wish as complete? You will earn XP!', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const result = await GamificationService.processWishCompletion(currentUserId, wish.id, wish.priority, wish.deadline);
            Alert.alert(
              'Wish Completed! 🎉', 
  ...
  return (
    <SafeAreaView style={styles.safeContainer} edges={['top']}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        {/* ✅ UPDATED TIME RING WITH LIVE COUNTDOWN */}
